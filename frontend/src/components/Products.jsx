          ...
          <div id={product.id} key={product.id} className="col-md-4 col-sm-6 col-12 mb-4">
            <div className="card h-100 shadow-sm border-0 rounded-4 text-center">
              ...
              <div className="card-body">
                <h5 className="card-title fw-semibold">{product.title.substring(0, 15)}...</h5>
                <p className="card-text text-muted" style={{ fontSize: "0.9rem" }}>
                  {product.description.substring(0, 90)}...
                </p>
              </div>
              <ul className="list-group list-group-flush">
                <li className="list-group-item bg-light fw-bold">${product.price}</li>
              </ul>
              <div className="card-body">
                <Link to={`/product/${product.id}`} className="btn btn-dark btn-sm me-2">
                  Buy Now
                </Link>
                <button className="btn btn-outline-dark btn-sm" onClick={() => addProduct(product)}>
                  Add to Cart
                </button>
              </div>
            </div>
